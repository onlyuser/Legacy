...
Vector Vector::toAngle()
{
	...
	if ((float) fabs(x) < EPSILON && (float) fabs(z) < EPSILON)
	{
		r.pitch = -sgn(y) * toRad(90);
		r.yaw = 0; // undefined
		return r;
	}
	if (x < 0) r.yaw *= -1;
...
float Vector::vec_maxDim()
{
	return max((float) fabs(x), max((float) fabs(y), (float) fabs(z)));
}

float Vector::vec_minDim()
{
	return min((float) fabs(x), min((float) fabs(y), (float) fabs(z)));
}
