...
void DexUser::load(HWND hWnd)
{
	...
	// build a tree-textured sprite, mask it with "tree_m.bmp"
	mDex3D_addSprite(
		"spr1", // mesh id
		10, // width
		10, // height
		true // it's a "tree-style" sprite
		);
	mDex3D_alignAxis("spr1", DIR::BOTTOM);
	mDex3D_modify("spr1", TRANS::ORIGIN, 0, 0, 0);
	mDex3D_setTexMap(
		"spr1", // mesh id
	...
	// build a lens1-textured sprite, mask it AGAIN with "lens1.bmp"
	mDex3D_addSprite(
		"spr2", // mesh id
		10, // width
		10, // height
		false // it's NOT a "tree-style" sprite
		);
	mDex3D_alignAxis("spr2", DIR::CENTER);
	mDex3D_modify("spr2", TRANS::ORIGIN, 10, 0, 0);
	mDex3D_setTexMap(
		"spr2", // mesh id
		(char *) (this->getPath() + "lens1.bmp").c_str(), // texture path
		(char *) (this->getPath() + "lens1.bmp").c_str(), // mask path
		-1, // mask color (-1 tells DexVT to get alpha from RGB data)
		DIR::FRONT // map direction
		);
