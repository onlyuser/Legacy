...
void DexUser::load(HWND hWnd)
{
	...
	// build 7 platonic primitives, color them randomly
	for (int i = 0; i < 7; i++)
	{
		// build a primitive, color it randomly
		std::string id = cstr("prim") + cstr(i + 1);
		switch (i)
		{
			case 0: mDex3D_addBox((char *) id.c_str(), 1, 1, 1); break;
			case 1: mDex3D_addCylndr((char *) id.c_str(), 1, 1, 16); break;
			case 2: mDex3D_addSphere((char *) id.c_str(), 1, 16, 16); break;
			case 3: mDex3D_addHemis((char *) id.c_str(), 1, 16, 16); break;
			case 4: mDex3D_addTorus((char *) id.c_str(), 2, 1, 16, 16); break;
			case 5: mDex3D_addOcta((char *) id.c_str(), 1); break;
			case 6: mDex3D_addTetra((char *) id.c_str(), 1);
		}
		mDex3D_setColor((char *) id.c_str(), RAND_COLOR);

		// offset the new primitive so it doesn't touch other primitives
		mDex3D_modify((char *) id.c_str(), TRANS::ORIGIN, i * 5, 0, 0);
	}
